...
implements RestrictedAccess, Threadable, Searchable {
    ...
    function  notifyCollaborators($entry, $vars = array(), $includeThread = false, $includeCollaborators = false) {
        global $cfg;
        ...
        $email->send($recipients, $cnotice['subj'], $body, $attachments,
            $options);
    }
   ...
   function onOverdue($whine=true, $comments="") {
    ...
    function addMessageThread($body, $currentId){
        ...
        // while ($row = $result->fetch_assoc()) {
        //     if($i > 0) {
        //         $body .= '<b>' . $row['poster'] . ' ' . date_format(date_create($row['created']), 'd-m-Y g:i A') . '</b><br/>';
        //         $body .= $row['body'] . '<br/><br/>';
        //     }                        
        //     else 
        //         $body .= "<br/><br/><h3>CONVERSACIONES ANTERIORES</h3>";
        ...
        foreach($this->getResponses() as $r) {
            ...
            if ($r->getId() != $currentId) {
                $body .= '<b>' . $r->getPoster() . ' ' . $r->getVar('create_date') . '</b><br/>';
                $body .= $r->getBody() . '<br/><br/>';

                $links = $this->getAttachmentsLinks($r);
                if($links != '')
                    $body .=  $links . '<br/>';
            }
    ...
    function getAttachmentsLinks($r, $file='attachment.php', $target='_blank', $separator=' ') {
        // TODO: Move this to the respective UI templates

        $str='';
        foreach ($r->getAttachments() as $att ) {
            ...
            $str .= sprintf(
                '&nbsp;&nbsp;&nbsp;<img src="http://www.dissertum.com/img/att.jpg" />&nbsp;<a href="%s" target="%s">%s</a>%s&nbsp;%s<br/>',
                'http://www.dissertum.com' . $att->file->getDownloadUrl(), $target,
    ...
    function addMessageCollaborators($body) {
        $i=0;
        $strEmails = 'Con copia a: ';
        foreach ($this->getRecipients() as $R2) {
            if($i > 0)
                $strEmails .= ', ';                        
            $strEmails .= $R2->getEmail();
            $i = $i + 1;
        }                

        if($i > 1)
            $body = $strEmails . '<br><br>' . $body;
